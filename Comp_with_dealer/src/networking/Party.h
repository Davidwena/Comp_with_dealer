...
// Concept: Type must be trivially copyable for direct memory transmission
template<typename T>
concept Transmittable = std::is_trivially_copyable_v<T> && 
                       (std::integral<T> || std::is_class_v<T>);
...
inline void Party::Send(std::size_t to_id, T message) {
    ...
    std::cerr << "Party " << my_id_ << " sent data to party " << to_id << '\n';
#endif
}
...
#ifdef MD_ML_DEBUG_ASIO
    std::lock_guard cerr_lock(cerr_mutex_);
    std::cerr << "Party " << my_id_ << " received data from party " << from_id << '\n';
#endif
